...
int main(int argc, char** argv) {
  ...
  int can_no=1;
  ROS_INFO("Started X for %s",nh.getNamespace().c_str());
  ...
  if (args.size() == 5) {
    ...
    ROS_ERROR("Usage: X <x> <y> <z> <yaw_deg> [<delay>]\n");
    return -1;
  }
  ...
  ROS_INFO("Publishing X on namespace %s: [%f, %f, %f].",
  ...
  mav_msgs::msgMultiDofJointTrajectoryFromPositionYaw(desired_position,
      desired_yaw, &trajectory_msg);
  ROS_INFO("Robot Drone %s going back to home at: [%f, %f, %f] after collecting the %d cans! ",nh.getNamespace().c_str(),x2,y2,z2,can_no);
  ros::Duration(3).sleep();
  trajectory_pub.publish(trajectory_msg);
